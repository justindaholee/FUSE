...
def process_cells(recent_cell: Dict[str, Union[int, float]],
                  current_frame: int,
    ...
    Args:
        recent_cell (Dict[str, Union[int, float]]): Dictionary containing details
            of the recent cell.
    ...
    Returns:
        List[Dict[str, Union[int, float]]]: List of dictionaries containing
            matching cell details and scores.
    """
    scores = []
    ...
    new_cell_coords = temp_df[temp_df['ROI'].isin(new_cells - 1)][['x', 'y']].to_numpy()
...
for i, mask in tqdm(enumerate(masks[1:]), total=len(masks)-1, leave=False,
    ...
    with ThreadPoolExecutor() as executor:
        results = executor.map(
            lambda recent_cell: process_cells(
                recent_cell,
                current_frame,
                temp_df,
                masks,
    ...
    lib.identify_cells(current_frame, all_scores)
